...
Pulibot.addCMD({pattern: 'news ?(.*)', fromMe: true, desc: Lang.NEWS_DESC}, async (message, match) => {
	...
	Pulibot.addCMD({pattern: 'news ?(.*)', fromMe: false, desc: Lang.NEWS_DESC}, async (message, match) => {
	if (match[1] === '') return await message.reply(Lang.NEED_CATEGORY);
	const url = `https://inshortsapi.vercel.app/news?category=${match[1]}`;
	try {
		const response = await got(url);
