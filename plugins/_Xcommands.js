...
if (Config.WORKTYPE == 'private') {
    ...
    Pulibot.addCMD({pattern: 'x ?(.*)', fromMe: true, dontAddCMDList: true}, (async (message, match) => {
    var CMD_HELP = '';
    if (match[1] === '') {
        Pulibot.commands.map(
            async (command) =>  {
    ...
    Pulibot.addCMD({pattern: 'x ?(.*)', fromMe: false, dontAddCMDList: true}, (async (message, match) => {
    var CMD_HELP = '';
    if (match[1] === '') {
        Pulibot.commands.map(
            async (command) =>  {
