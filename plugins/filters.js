...
# All rights reserved ©pulithasethnindu
...
Pulibot.addCMD({pattern: 'filter ?(.*)', fromMe: true, desc: Lang.FILTER_DESC}, (async (message, match) => {
...
Pulibot.addCMD({pattern: 'stop ?(.*)', fromMe: true, desc: Lang.STOP_DESC}, (async (message, match) => {
...
Pulibot.addCMD({on: 'text', fromMe: false}, (async (message, match) => {
...
Pulibot.addCMD({on: 'text', fromMe: true, deleteCommand: false, dontAddCMDList: true}, (async (message, match) => {
    var filtreler = await FilterDb.getFilter(message.jid);
    if (!filtreler) return; 
    return filtreler.map(
        async (filter) => {
